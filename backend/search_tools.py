...
class CourseOutlineTool(Tool):
    """Tool for retrieving complete course outlines with metadata and lesson lists"""
    ...
    def get_tool_definition(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": "Get complete course outline with metadata and lesson list",
            "input_schema": {
                "type": "object",
                "properties": {
                    "course_title": {
    ...
    def execute(self, course_title: str) -> str:
        """
        Execute the outline tool to retrieve course information.
        ...
        # Step 1: Resolve the course name using vector search
    ...
    def _format_course_outline(
        ...
        # Course header
        outline.append(f"**{course_title}**")
        if instructor:
            outline.append(f"Instructor: {instructor}")
        if course_link:
            outline.append(f"Course Link: {course_link}")

        outline.append(f"Total Lessons: {lesson_count}")
        outline.append("")  # Empty line
